...
def zhuagui():
    ...
    if rect:
        rect[0] = rect[0] + 280 * 1536 / 1920
        do(rect, '点击捉鬼活动')
        time.sleep(5)
    else:
        autopy.mouse.smooth_move(rect1[0] + 200 * 1536 / 1920, rect1[1])
        time.sleep(1)
        while True:
            win32api.mouse_event(win32con.MOUSEEVENTF_WHEEL, 0, 0, -200* 1536 / 1920)
            rect = matcher.match_sub_image(startx, starty, window_capture_all(), './image/zhuogui/zhuogui.png')
            if rect:
                break
    ...
    while (True):
        ...
        if rect:
            ...
            if rect:
                ...
                rect = matcher.match_sub_image(startx, starty, window_capture_all(),
                                               './image/zhuogui/lingquzhuogui.png')
                do(rect, '领取捉鬼任务')
                time.sleep(2)
                ...
                if rect:
                    ...
                    rect = matcher.match_sub_image(startx, starty, window_capture_all(),
                                                   './image/zhuogui/jxzhuogui.png')
                    do(rect, '确定不满5人')
                    time.sleep(3)

                    rect = matcher.match_sub_image(startx, starty, window_capture_all(),
                                                   './image/zhuogui/zidongpipei.png')
                    do(rect, '点击自动匹配')
                    time.sleep(1)
                    ...
                    rect1 = matcher.match_sub_image(startx, starty, window_capture_all(),
                                                    './image/zhuogui/richanghuidong.png')
                    do(rect1, '选择日常活动')
                    time.sleep(1)
                    ...
                    rect = matcher.match_sub_image(startx, starty, window_capture_all(),
                                                   './image/zhuogui/zhuoguirenwu1.png')
                    do(rect, '点击捉鬼任务1')
                    time.sleep(2)
                    do(rect, '点击捉鬼任务2')
                    time.sleep(1)
...
class Frame(wx.Frame):
    def __init__(self, parent=None, id=wx.ID_ANY, title=u"梦幻手游辅助v" + ver, pos=wx.DefaultPosition, size=(500, 400),
        ...
        self.tripText2 = wx.StaticText(panel, wx.ID_ANY,
                                       label=u'--------------------------------------操作--------------------------------------------',
                                       pos=(0, 35))
        ...
        self.hanhua = wx.Button(panel, wx.ID_ANY, label=u"微信喊话", pos=(10, 190), size=(110, 35))
        # 控件事件
        ...
        self.Bind(wx.EVT_BUTTON, hanhuaTh, self.hanhua)
...
def hanhuaTh(param):
    global frame
    if frame.hanhua.LabelText == '微信喊话':
        frame.jinyan.LabelText = '停止喊话'
        AddToList('微信喊话')
        # 检查口令
        # if(CheckPwd() != True):
        #     return False

        t = threading.Thread(target=hanhua)
        t.setDaemon(True)
        t.start()

    else:
        frame.hanhua.LabelText = '微信喊话'
        AddToList('停止喊话')
        isjinyan = False


def hanhua():
    rect = matcher.match_sub_image(startx, starty, window_capture_all(), './image/wx/img.png')
    print(rect)
    time.sleep(3)
    autopy.mouse.move(rect[0] * 1536 / 1920, (rect[1] - 40) * 864 / 1080)
    autopy.mouse.click()
    keyboard(rect, "123")
    time.sleep(1)
    do(rect, '微信喊话')
...
def keyboard(rect, msg):
    if rect:
        autopy.key.type_string(msg)
        AddToList(msg)
    else:
        AddToList(msg + '--stop')
        return
...
def do(rect, msg):
    if rect:
        autopy.mouse.move(rect[0] * 1536 / 1920, (rect[1]) * 864 / 1080)
        autopy.mouse.click()
        AddToList(msg)
    else:
        AddToList(msg + '--stop')
